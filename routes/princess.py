...
def princess_diaries():
    ...
    # Precompute shortest paths for relevant stations
    relevant_stations = set([start_station] + [t["station"] for t in tasks])
    dist_map = {s: dijkstra(max_station, graph, s) for s in relevant_stations}

    # DP: dp[i] = (score, fee_without_return, schedule)
    dp = [(0, 0, [])]
    ...
    for i, task in enumerate(tasks, start=1):
        ...
        if not prev_sched:
            # cost from start to first task
            new_fee = dist_map[start_station][task["station"]]
        else:
            last_station = prev_sched[-1]["station"]
            dist_leg = dist_map[last_station][task["station"]]
            if dist_leg >= INF:
                new_fee = INF
            else:
                new_fee = prev_fee + dist_leg
        ...
        if take[0] > skip[0]:
            ...
            # Tie on score → pick min fee
            if take[1] < skip[1]:
                dp.append(take)
            else:
                dp.append(skip)

    # Finalize: add return to start
    best_score, fee_no_return, best_schedule = dp[-1]
    if best_schedule:
        last_station = best_schedule[-1]["station"]
        back_cost = dist_map[last_station][start_station]
        best_fee = fee_no_return + back_cost if back_cost < INF else INF
        best_names = [t["name"] for t in sorted(best_schedule, key=lambda x: x["start"])]
    else:
        best_fee = 0
        best_names = []
